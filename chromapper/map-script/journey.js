...
function run(cursor, notes, events, walls, _, global, data, customEvents, bpmChanges) {
    ...
    randomizeColorStack(between(notes, 454 + 192, 513 + 192), [200, 0.875, 0.875], [210, 0.875, 0.875]);
    ...
    randomizeColorStack(between(notes, 454 + 448, 513 + 448), [200, 0.875, 0.875], [210, 0.875, 0.875]);
    ...
    for (const tht of towerHitTiming) {
        setColor(where(at(notes, tht[0]), { include: { _lineLayer: tht[1] ? 0 : 2 } }), [0, 0, 0.375]);
    ...
    setGradientColor(where(between(bombs, 834, 837), { include: { _lineIndex: 0 } }), [180, 0, 0.375], [180, 0, 1]);
    setGradientColor(where(between(bombs, 834, 837), { include: { _lineIndex: 1 } }), [195, 0, 1], [195, 0, 0.375]);
    setGradientColor(where(between(bombs, 834, 837), { include: { _lineIndex: 2 } }), [210, 0, 0.375], [210, 0, 1]);
    setGradientColor(where(between(bombs, 834, 837), { include: { _lineIndex: 3 } }), [225, 0, 1], [225, 0, 0.375]);
