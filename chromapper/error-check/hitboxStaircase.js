      ...
      if (kvlCore.isNote(note) && lastNote[note._type]) {
         if (
            ...
            if (note._cutDirection !== 8) {
               noteOccupy[note._type]._lineIndex =
                  note._lineIndex + kvlCore.swingCutDirectionSpace[note._cutDirection][0];
               noteOccupy[note._type]._lineLayer =
                  note._lineLayer + kvlCore.swingCutDirectionSpace[note._cutDirection][1];
            } else {
            ...
            kvlCore.swingNoteEnd(note, lastNote[note._type], lastNoteDirection[note._type])
         ) {
            if (note._cutDirection !== 8) {
               noteOccupy[note._type]._lineIndex =
                  note._lineIndex + kvlCore.swingCutDirectionSpace[note._cutDirection][0];
         ...
         if (
            ...
            kvlCore.isBelowThres(
               ...
               Math.min(kvlCore.toolValue.hitbox.staircase, lastSpeed[(note._type + 1) % 2]),
               data.songBPM,
            )
         ) {
            if (
               ...
               addWarning(lastNoteEnd[(note._type + 1) % 2], 'Note causing the staircase hit');
