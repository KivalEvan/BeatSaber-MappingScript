      ...
      if (!noRot) {
         randCD =
            validRotation[notes[i]._type][parity[notes[i]._type]][
               Math.floor(
                  Math.random() * validRotation[notes[i]._type][parity[notes[i]._type]].length,
               )
            ];
         if (prevRotation[notes[i]._type] !== null) {
            let tempArr = validRotation[notes[i]._type][parity[notes[i]._type]].filter((cd) =>
               kvlCore.checkAngle(cd, prevRotation[notes[i]._type], Math.abs(180 - maxAngle), true),
            );
            if (tempArr.length) {
