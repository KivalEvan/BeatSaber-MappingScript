...
const getKeysound = (key: Keysound, index = 0, offset = 0): KeysoundNote[] => {
   ...
   let samSet = sampleSet[
      key.type === 'slider' && key.edgeSet?.[index][0]
         ? key.edgeSet[index][0]
         : (key.hitSample?.[0] ||
            timingPoints.find((tp) => !tp.uninherited && tp.time <= key.time + offset)!
               .sampleSet) ??
            0
   ];
   if (samSet === 'auto') {
   ...
   if (
      ...
      if (
         !keysoundMap.find(
            (kms) =>
               kms._file ===
                  `${samSet}-hit${hitSample[1]}${sampleIndex > 1 ? sampleIndex : ''}.wav`,
         )
      ) {
         keysoundMap.push({
            _id: id++,
      ...
      ary.push({
         ...
         _type: 'hit',
         _volume: timingPoints.find((tp) =>
            !tp.uninherited && tp.time <= key.time + offset
         )!.volume ??
            1,
         _pitch: 1,
         _offset: 0,
   ...
   samSet = sampleSet[
      key.type === 'slider' && key.edgeSet?.[index][1]
         ? key.edgeSet[index][1]
         : (key.hitSample?.[1] ||
            timingPoints.find((tp) => !tp.uninherited && tp.time <= key.time + offset)!
               .sampleSet) ??
   ...
   sampleIndex = (key.hitSample?.[2] ||
      ...
      1;
   if (
      (key.type === 'slider' && typeof key.edgeSound?.[index] === 'number' &&
         key.edgeSound[index] & 0x02) ||
      key.hitSound & 0x02
   ) {
      if (
         !keysoundMap.find(
            (kms) =>
               kms._file ===
                  `${samSet}-hit${hitSample[2]}${sampleIndex > 1 ? sampleIndex : ''}.wav`,
         )
      ) {
         keysoundMap.push({
            _id: id++,
   ...
   if (
      ...
      if (
         !keysoundMap.find(
            (kms) =>
               kms._file ===
                  `${samSet}-hit${hitSample[4]}${sampleIndex > 1 ? sampleIndex : ''}.wav`,
         )
      ) {
         keysoundMap.push({
            _id: id++,
   ...
   if (
      ...
      if (
         !keysoundMap.find(
            (kms) =>
               kms._file ===
                  `${samSet}-hit${hitSample[8]}${sampleIndex > 1 ? sampleIndex : ''}.wav`,
         )
      ) {
         keysoundMap.push({
            _id: id++,
...
const sliderMultiplier = parseFloat(
   mapped.Difficulty.find((s: string) => s.startsWith('SliderMultiplier'))!
      .split(
         ':',
      )[1],
);
...
const toBeatSaberNote = (key: Keysound): void => {
   ...
   if (key.type === 'slider') {
      const sliderMs = closestValue(
         (((600 * key.length) / (BPM * sliderMultiplier) / 1000) * BPM) / 60,
         [
            16,
            24,
         ],
         ...
         getMultiplier(
            timingPoints.find((tp) => !tp.uninherited && tp.time <= key.time)!
               .beatLength,
         );
      for (let i = 0; i <= key.backAndForth; i++) {
         notes.push({
            _time: closestValue(((key.time + offset) * BPM) / 60, [16, 24]) +
               sliderMs * i + 1,
            _lineIndex: i ? 0 : 1,
            _lineLayer: 0,
            _type: 0,
