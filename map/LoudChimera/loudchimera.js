...
const difficulty = JSON.parse(fs.readFileSync(INPUT));
...
function setWallNJS(t1, t2, speed, jd) {
    ...
    const filteredwalls = _obstacles.filter((w) => w._time >= t1 && w._time <= t2);
...
function simultaneousNoteSpawn(t1, t2, speed, njsoff = NJSOFFSET) {
    ...
    const filterednotes = _notes.filter((n) => n._time >= t1 && n._time <= t2);
    const time = filterednotes[0]._time;
    return filterednotes.forEach((note) => {
...
function simultaneousWallSpawn(t1, t2, speed, njsoff = NJSOFFSET) {
    ...
    const filteredwalls = _obstacles.filter((w) => w._time >= t1 && w._time <= t2);
    const time = filteredwalls[0]?._time;
...
const fastPewPew = [].concat(getRepeatArray(394, 16, 8), getRepeatArray(906, 16, 8));
...
const fourWideBigSlap = [640, 1152];
...
const glitchyBurst = [].concat(getRepeatArray(526, 32, 4), getRepeatArray(1038, 32, 4));
