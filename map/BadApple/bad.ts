import { BeatPerMinute, ext, globals, load, save, types, v3 } from '../../depsLocal.ts';
import * as imagescript from 'https://deno.land/x/imagescript@v1.2.12/mod.ts';
import { interleave } from '../../../BeatSaber-Deno/utils/misc.ts';
...
for (let id = 0; id < 4; id++) {
    difficulty.addLightTranslationEventBoxGroups({
        time: 0,
        id,
        events: [{ axis: 1, events: [{ translation: -5 }] }],
    });
}
...
for (let id = 20; id < 22; id++) {
    difficulty.addLightTranslationEventBoxGroups({
        ...
        events: [
            ...
            { filter: { type: 2, p0: 2 }, axis: 1, events: [{ translation: 2.5 }] },
