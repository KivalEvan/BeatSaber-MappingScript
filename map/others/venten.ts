...
between(where(lightshow.basicEvents, { include: { _type: 4 } }), 2, 6).forEach(
    (ev, i, _) => ((ev.floatValue = i / (_.length * 2)), makeWhite(ev)),
);
where(between(lightshow.basicEvents, 34, 37.99), { exclude: { _type: 1 } }).forEach(
    (ev, i, _) => (ev.floatValue = i / (_.length * 2) + 0.75),
);
where(between(lightshow.basicEvents, 386, 389.99), { exclude: { _type: 4 } }).forEach(
    (ev, i, _) => (ev.floatValue = i / _.length + 0.25),
);
where(between(lightshow.basicEvents, 68.75, 69.99), { include: { _type: 0 } }).forEach((ev) => makeWhite(ev, 0.75));

where(between(lightshow.basicEvents, 101, 101.99), { include: { _type: 4 } }).forEach((ev) => makeWhite(ev, 0.75));
where(between(lightshow.basicEvents, 129, 129.99), { include: { _type: 4 } }).forEach((ev) => makeWhite(ev, 0.75));
where(between(lightshow.basicEvents, 134, 162), { include: { _type: 4 } })
    .filter((ev) => ev.isBlue())
    .forEach((ev) => makeWhite(ev, 0.75));
where(at(lightshow.basicEvents, 162), { include: { _type: 1 } }).forEach((ev) => makeWhite(ev));

where(between(lightshow.basicEvents, 261, 261.99), { include: { _type: 4 } }).forEach((ev) => makeWhite(ev, 0.75));
where(between(lightshow.basicEvents, 289, 289.99), { include: { _type: 4 } }).forEach((ev) => makeWhite(ev, 0.75));
where(between(lightshow.basicEvents, 294, 322), { include: { _type: 4 } })
    .filter((ev) => ev.isBlue())
    .forEach((ev) => makeWhite(ev, 0.75));
where(at(lightshow.basicEvents, 322), { include: { _type: 1 } }).forEach((ev) => makeWhite(ev));
...
where(at(lightshow.basicEvents, 454), { include: { _type: 4 } }).forEach((ev) => makeWhite(ev));
...
for (const fi of flashIt) {
    at(where(lightshow.basicEvents, { include: { _type: 4 } }), fi).forEach((ev) => makeWhite(ev));
}
