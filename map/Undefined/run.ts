    ...
    for (const d of set._difficultyBeatmaps) {
        if (d._customData) {
            ...
            if (d._difficulty === 'Expert') {
                d._customData._information.splice(1, 0, 'Flower Sign "Blossoming of Gensokyo"');
            }
            if (d._difficulty === 'ExpertPlus') {
                if (set._beatmapCharacteristicName === 'Standard') {
                    d._customData._information.splice(1, 0, 'Fantasy "The Beauties of Nature"');
                } else {
                    d._customData._information.splice(1, 0, '"Fantastic Spring Flowers"');
                }
            }
        ...
        if (d.characteristic === 'OneSaber') {
            if (n.time >= 98.25 + j * 4 && n.time <= 100.5 + j * 4) {
                n.angleOffset = Math.round(
                    utils.lerp(utils.normalize(n.time, 98.25 + j * 4, 100.5 + j * 4), -22.5, 22.5) * (j % 2 ? 1 : -1),
                );
            }
            if (n.time >= 100.5 + j * 4) {
                j++;
