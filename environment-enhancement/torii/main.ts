...
export const generateEnvironment = (): types.v3.IChromaEnvironment[] => {
   ...
   cubeBlock.place(
      {
         position: [
            -toriiWidth / 2 - toriiWidthExt / 2 - 0.625,
            toriiHeight + 1.02,
            0,
         ],
         scale: [1.1875, 0.5, 1],
   ...
   const lanternPostGroup = ext.chroma.EnvironmentGroup.create(lanternPost, [
      0,
      0,
      0,
   ]);
   for (let z = 0; z < toriiCount * lanternPostSection; z++) {
      const toriiZ = Math.floor(
         Math.abs((lanternPostOffset + z) / lanternPostSection),
      );
   ...
   cubeBlock.place(
      {
         position: [
            lanternSize / 2,
            lanternBaseHeight / 2 + lanternHeight / 2 + lanternGap / 4 +
            lanternBorder / 2,
            lanternSize / 2,
         ],
         scale: [
   ...
   cubeBlock.place(
      {
         position: [
            lanternSize / 2,
            lanternBaseHeight + lanternHeight + lanternGap / 2,
            0,
         ],
   ...
   cubeLight.place(
      {
         position: [
            0,
            lanternBaseHeight + lanternGap / 2 + lanternHeight / 2,
            0,
         ],
   ...
   for (let i = 0; i < 9; i++) {
      ...
      if (i === 0) {
         regexPillarL = EnvGrab.create('BottomPairLasers').child().name('PillarL').end()
            .regex;
         regexPillarR = EnvGrab.create('BottomPairLasers').child().name('PillarR').end()
            .regex;
      } else {
